...
class OutputViewController: UIViewController {
    ...
    func setUpView() {
        Utilities.styleTextField(outputText)
    ...
    @IBAction func submitTapped(_ sender: Any) {
        ...
        // clean data
        let output = outputText.text!.trimmingCharacters(in: .whitespacesAndNewlines)
        // get patient UID
        ...
        db.collection("Nurses").document(uid).collection("Current Patient").document("Patient Info").getDocument { (snapshot, error) in
            ...
            // add output data
            db.collection("Patients").document("\(patient_uid)").collection("Output").addDocument(data: ["output": output, "Time": NSDate()])
            // error message
