...
class PositioningViewController: UIViewController {
    ...
    @IBOutlet weak var positioningText: UITextField!
    ...
    func setUpView() {
        Utilities.styleTextField(positioningText)
    ...
    @IBAction func submitTapped(_ sender: Any) {
        // clean data
        let position = positioningText.text!.trimmingCharacters(in: .whitespacesAndNewlines)
        // get patient UID
        ...
        db.collection("Nurses").document(uid).collection("Current Patient").document("Patient Info").getDocument { (snapshot, error) in
            ...
            // add postion data
            db.collection("Patients").document("\(patient_uid)").collection("Positioning").addDocument(data: ["Positioning": position, "Time": NSDate()])
            // error message
            ...
            // clear text field
            self.positioningText.text = ""
        }
        print("sucessfully submitted position data")
    }
